...
@class LCLANDeviceFileVideoItem;
@class LCOpenPlayTokenModel;
@protocol LCMediaPlayerListener;

NS_ASSUME_NONNULL_BEGIN

@interface LCMediaPlayer : NSObject
//开放平台playToken解析model
 ...
 *  设置监听者，用来接收播放状态回调
 *
 *  @param lis 监听者实例(需遵守LCMediaPlayerListener协议)
 */
...
- (NSInteger) playLANRealTimeStream:(LCLANLiveVideoItem *)videoItem;
...
/// 局域网私有协议按时间回放拉流
/// @param videoItem 视频源
- (NSInteger)playLANDevicePrivateByTime:(LCLANDeviceTimeVideoItem *)videoItem;

/// 局域网并行按时间回放拉流
/// @param videoItem 视频源
- (NSInteger)playLANDeviceByTime:(LCLANDeviceTimeVideoItem *)videoItem;

/// 局域网私有协议按文件回放拉流
/// @param videoItem 视频源
- (NSInteger)playLANDevicePrivateByFile:(LCLANDeviceFileVideoItem *)videoItem;
...
/// 云图播放接口
/// - Parameter imagesItem: 参数类(LCCloudImagesItem)
- (NSInteger) playCloudImageStream:(LCCloudImagesItem *)imagesItem;
 ...
 *  @note   该接口为异步接口
 */
- (NSInteger) playCloudDiskRecordStream:(NSString*)deviceSN bindDeviceId:(NSString*)bindDeviceId channelId:(NSInteger)channelId recordId:(int64_t)recordId recordType:(E_CLOUD_RECORD_TYPE)recordType hlsType:(E_HLS_TYPE)hlsType startTime:(NSInteger)startTime timeout:(NSInteger)timeout isEncrypt:(NSInteger)isEncrypt PSK:(NSString*)PSK Region:(NSString*) region RecordPath:(NSString *) recordPath Speed:(CGFloat)speed Username:(NSString*) strUserName PSW:(NSString*) strPassWord;
...
/// 设置主动停流标示
-(void)setAnalysisDataResCancel;

/// 设置拉流超时标示
-(void)setAnalysisDataResTimeout;
...
/// @param width 辅助框宽度
/// @param height 辅助框高度
- (void) setAssistViewSize:(int)width Height:(int)height;

/**
 * 设置自动追踪默认区域大小
 * @param centerX     [in]  默认区域中心点X坐标
 * @param centerX     [in]  默认区域中心点Y坐标
 * @param XSize       [in]  默认区域中心点距左右边界的距离
 * @param YSize       [in]  默认区域中心点距上下边界的距离
 */
