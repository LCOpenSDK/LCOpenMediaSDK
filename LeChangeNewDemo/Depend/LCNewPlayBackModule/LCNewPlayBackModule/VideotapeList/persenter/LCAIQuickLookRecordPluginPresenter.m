...
@implementation LCAIQuickLookRecordPluginPresenter
...
- (instancetype)initWithHostViewController:(UIViewController *)hostViewController onPlayTime:(void (^)(NSTimeInterval))onPlayTime {
    ...
    if (self) {
        ...
        _onPlayTime = [onPlayTime copy];
        _expectFirstSDKLoadingEvent = YES;
    }
    return self;
}

- (void)resetExpectFirstSDKLoadingForNewPlay {
    self.expectFirstSDKLoadingEvent = YES;
}
...
- (void)onPlaySuccess:(LCBaseVideoItem *)videoItem {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *v = self.hostViewController.view;
        [LCProgressHUD hideAllHuds:v ?: [UIApplication sharedApplication].keyWindow];
        if (self.onBizState) {
            self.onBizState(LCAICloudQuickLookBizSceneNone, nil);
        }
        if (self.onChromeRefresh) {
            self.onChromeRefresh();
        }
    });
}
...
- (void)onPlayLoading:(LCBaseVideoItem *)videoItem {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.onBizState) {
            if (self.expectFirstSDKLoadingEvent) {
                self.expectFirstSDKLoadingEvent = NO;
...
- (void)onSingleClick:(UITapGestureRecognizer *)gesture cid:(NSInteger)cid {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.onSingleTapVideo) {
            self.onSingleTapVideo(cid);
        }
    });
}

- (void)onDoubleClick:(UITapGestureRecognizer *)gesture cid:(NSInteger)cid {
    if (!self.onDoubleTapVideo) {
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
        self.onDoubleTapVideo(cid);
    });
}
